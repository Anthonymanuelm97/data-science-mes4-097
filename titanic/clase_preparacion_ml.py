...
orden_edad = ["Niño", "Adolescente", "Adulto Jovem", "Adulto", "Mayor"]
df["AgeGroup"] = pd.Categorical(
    df["AgeGroup"], categories=orden_edad, ordered=True)

print("Dataset cargado: ", df.shape)
print("Pclass: ", df["Pclass"].dtypes)
print("AgeGroup: ", df["AgeGroup"].dtypes)

# Columnas a descartar

columnas_descartar = ["PassengerId", "Name", "Ticket"]
df_ml = df.drop(columns=columnas_descartar)
...
print(" X - features que el modelo usara para aprender: ")
print("   shape:", X.shape)
print("   columnas:", list(X.columns))
print()
print("La variable target 'y' es la que el modelo intentara predecir")
print("   shape:", y.shape)
...
X_train_enc = pd.get_dummies(
    X_train, columns=columnas_categoricas, drop_first=False)
X_test_enc = pd.get_dummies(
    X_test, columns=columnas_categoricas, drop_first=False)
...
columnas_numericas = ["Age", "SibSp", "Parch", "Fare", "FamilySize", "IsAlone"]

scaler = StandardScaler()

X_train_enc[columnas_numericas] = scaler.fit_transform(
    X_train_enc[columnas_numericas])

X_test_enc[columnas_numericas] = scaler.transform(
    X_test_enc[columnas_numericas])

print("Estadistica de columna numerca X_train:")
...
print("Tipos de datos numericos en X_train")
